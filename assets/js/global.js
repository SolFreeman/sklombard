jQuery(document).ready(function ($) {
	...
	$(window).on('load scroll', function () {
		...
		var listTop = $('.commission__photo-line').offset().top;

		if (scrollTop + windowHeight > listTop) {
			var delay = 0;
			$('.commission__photo').each(function () {
	...
	// Функция для пересчета высоты суб контента
    function setSubcontentHeight() {
        if ($(window).width() >= 768) {
			var height = $('.active > .sub-content').outerHeight();
			$('.category-holder').height(height);
		} else {
			$('.category-holder').height('');
		}
    }
    ...
    $('.category-holder .item').click(function() {
        setSubcontentHeight();
    });
